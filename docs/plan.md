...
| `lifecycle` | Interactive "start the stack?"; compose/image bring-up and tear-down (`down`/`stop`); rebuild-on-drift (recreate when a stack file is newer than the container); run-once `postCreateCommand` + marker stamping. |
